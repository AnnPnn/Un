...
class DbManager {
    ...
    fun publishAd(ad: Ad, finishListener: FinishWorkListener) {
        if (auth.uid != null)
            db.child(ad.key ?: "empty").child(auth.uid!!).child(AD_NODE)
    ...
    private fun readDataFromDb(query: Query, readDataCallback: ReadDataCallback?) {
        query.addListenerForSingleValueEvent(object : ValueEventListener {
            ...
            override fun onDataChange(snapshot: DataSnapshot) {
                val adArray = ArrayList<Ad>()//массив для перезаписи данных
            ...
            }//Функция считывает информацию и выдает в виде объекта DataSnapshot
    ...
    }//абстракная функция используемая в DbManager
    ...
    }//константы хранящиеся в данном классе
}
